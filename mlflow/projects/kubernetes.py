...
class KubernetesSubmittedRun(SubmittedRun):
    ...
    def _update_status(self, kuber_api):
    ...
    def get_status(self):
        status = self._status
        kuber_api = kubernetes.client.BatchV1Api()
        return status if RunStatus.is_terminated(status) else self._update_status(kuber_api)
